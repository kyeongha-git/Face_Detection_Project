...
parser = argparse.ArgumentParser(description="OurModel Training")
parser.add_argument(
    "--training_dataset",
...
optimizer = optim.SGD(
    net.parameters(), lr=initial_lr, momentum=momentum, weight_decay=weight_decay
)
...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 10 == 0 and epoch > 0) or (
                epoch % 5 == 0 and epoch > cfg["decay1"]
            ):
                torch.save(
                    net.state_dict(),
        ...
        print(
            "Epoch:{}/{} || Epochiter: {}/{} || Iter: {}/{} || Loc: {:.4f} Cla: {:.4f} Landm: {:.4f} || LR: {:.8f} || Batchtime: {:.4f} s || ETA: {}".format(
                epoch,
                max_epoch,
