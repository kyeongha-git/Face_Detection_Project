...
from src.backbone.mobilenetv1 import MobileNetV1
from src.layers.wfpn import WFPN
from src.layers.scm import SCM
from src.layers.eca_cbam import ECA_CBAM
...
class OurModel(nn.Module):
    def __init__(self, cfg=None, phase="train"):
        ...
        self.body = _utils.IntermediateLayerGetter(
            backbone, return_layers=cfg["return_layers"]
        )
        ...
        self.attn_backbone = nn.ModuleList(
            [ECA_CBAM(channel=ch) for ch in in_channels_list]
        )
        ...
        self.attn_wfpn = nn.ModuleList(
            [ECA_CBAM(channel=out_channels) for _ in range(3)]
        )

        # Context module
        self.context = nn.ModuleList(
            [SCM(out_channels, out_channels) for _ in range(3)]
        )

        # Attention: context output
        self.attn_context = nn.ModuleList(
            [ECA_CBAM(channel=out_channels) for _ in range(3)]
        )
    ...
    def forward(self, inputs):
        ...
        bbox_regressions = torch.cat(
            [self.bbox_head[i](feature) for i, feature in enumerate(features)], dim=1
        )
        classifications = torch.cat(
            [self.class_head[i](feature) for i, feature in enumerate(features)], dim=1
        )
        ldm_regressions = torch.cat(
            [self.landmark_head[i](feature) for i, feature in enumerate(features)],
            dim=1,
        )
